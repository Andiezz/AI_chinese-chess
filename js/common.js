...
com.init = function (stype){
	...
	com.width			=	stype.width;		//canvas width
	com.height			=	stype.height; 		//canvas height
	com.spaceX			=	stype.spaceX;		//point X span
	com.spaceY			=	stype.spaceY;		//point Y span
	com.pointStartX		=	stype.pointStartX;	//X coordinate of the first point of impact;
	com.pointStartY		=	stype.pointStartY;	//The first to point Y coordinate;
	com.page			=	stype.page;			//picture directory
	...
	com.loadImages(com.page);		//Load images/image directory
	//z(com.initMap.join())
}
...
com.stype = {
	stype1:{
		width:325,		//canvas width
		height:402, 		//canvas height
		spaceX:35,		//point X span
		spaceY:36,		//point Y span
		pointStartX:5,		//X coordinate of the first point of impact
		pointStartY:19,		//Y coordinate of the first point of impact;
		page:"stype_1"	//picture directory
	},
	stype2:{
		width:530,		//canvas width
		height:567, 		//canvas height
		spaceX:57,		//point X span
		spaceY:57,		//point Y span
		pointStartX:-2,		//X coordinate of the first point of impact
		pointStartY:0,		//Y coordinate of the first point of impact;
		page:"stype_2"	//picture directory
	}		
}
//get id
com.get = function (id){
...
window.onload = function(){  
	...
	com.mans	 ={};		//collection of chess pieces
	com.createMans(com.initMap)		//generate pawns
	com.bg.show();
	com.get("bnBox").style.display = "block";
	//play.init();
	com.get("billBn").addEventListener("click", function(e) {
		if (confirm("Whether to end the game and start chess game research")){
			com.init();
	...
	com.get("superPlay").addEventListener("click", function(e) {
		if (confirm("Are you sure to start the hard game?")){
			play.isPlay=true ;	
	...
	com.get("tyroPlay").addEventListener("click", function(e) {
		if (confirm("Are you sure to start the easy level game?")){
			play.isPlay=true ;	
			com.get("chessRight").style.display = "none";
			com.get("moveInfo").style.display = "block";
			com.get("moveInfo").innerHTML="";
...
com.createMove = function (map,x,y,newX,newY){
	...
	delete map[y][x];
	if (man.my===1){
		var mumTo=["one","two","three","four","five","six","seven","eight","nine","ten"];	
		newX=8-newX;
		h+= mumTo[8-x];
		if (newY > y) {
			h+= "Retreat";
